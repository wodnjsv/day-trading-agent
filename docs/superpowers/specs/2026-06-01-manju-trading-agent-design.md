...
- **Phase 1 — 실행 코어 + 짝꿍 전략 (10만원 plumbing 검증)**
  - SHARED STATE, SignalEngine, OrderManager, PositionManager, RiskGovernor, PairFollowStrategy. 10만원 소액 실거래로 주문/체결/손절/장애 배관 검증.
- **Phase 2 — 백테스트 하네스**
  - ReplayFeed + 체결 시뮬레이터. 축적 데이터로 짝꿍 전략 로직·수익성 검증.
- **Phase 3 — 두뇌 레이어 + 학습 루프**
  - RegimeAnalyzer/ThemeCurator/NewsInterpreter/Reviewer + TradingPlan + EventDetector + LearningStore.
- **Phase 4 — 점진 증액 + 전략 확장**
  - 검증된 전략부터 비중 증액, 낙주·상따 등 플러그인 추가.
